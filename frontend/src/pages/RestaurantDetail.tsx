...
function computeCriteriaAverages(ratings: any[]): Record<string, number> | null {
  if (!ratings || ratings.length === 0) return null;
  const sums: Record<string, number> = {};
  const counts: Record<string, number> = {};
  for (const key of CRITERIA_KEYS) {
    const dbKey = CRITERIA_DB_MAP[key];
    sums[key] = 0;
    counts[key] = 0;
    for (const r of ratings) {
      const val = parseFloat(r[dbKey]);
      if (!isNaN(val)) {
        sums[key] += val;
        counts[key]++;
      }
...
export default function RestaurantDetail() {
  ...
  const score = displayRestaurant.clean_score;
  ...
  return (
    <div className="flex-1 pb-24 max-w-lg mx-auto w-full">
      ...
      <div className="bg-gradient-to-br from-teal-600 to-teal-500 px-5 pb-12" style={{ paddingTop: 'calc(env(safe-area-inset-top, 0px) + 1.25rem)' }}>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            ...
            >
              <svg className="w-4 h-4 text-white" viewBox="0 0 24 24" fill={isFavorite ? 'white' : 'none'} stroke="currentColor" strokeWidth={2}>
        ...
        <h1 className="text-2xl font-bold text-white tracking-[-0.5px]">{baseRestaurant.name}</h1>
        {(baseRestaurant.address || baseRestaurant.cuisine) && (
          <p className="text-sm text-white/80 mt-1">
            {[baseRestaurant.address, baseRestaurant.cuisine].filter(Boolean).join(' · ')}
          </p>
        )}
      </div>
      ...
      <div className="mx-5 -mt-6 relative z-10">
        <motion.div
          className="bg-white rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.06)] p-5"
            ...
            <div className="flex gap-5">
              ...
              {criteriaAvgs ? (
                <div className="flex-1 grid grid-cols-1 gap-2.5">
                  {CRITERIA_KEYS.map((key, i) => {
                    ...
                    return (
                      ...
                      >
                        <span className="text-[11px] text-stone-400 w-20 shrink-0 truncate">
                ...
                <div className="flex-1 flex items-center">
                  <p className="text-[13px] text-stone-400">{t('restaurant.noDetailedScores', 'Detailbewertungen nicht verfügbar')}</p>
                </div>
              )}
            </div>
          ) : (
            <div className="text-center py-3">
              <div className="w-16 h-16 rounded-[18px] bg-stone-100 flex items-center justify-center mx-auto mb-2">
                <span className="text-2xl font-extrabold text-stone-300">?</span>
              </div>
              <p className="text-sm text-stone-400">{t('restaurant.noReviews', 'Noch keine Bewertungen')}</p>
            </div>
          )}
        </motion.div>
      </div>
      ...
      <div className="mx-5 mt-4">
        <motion.button
          ...
          className="w-full py-3.5 rounded-xl bg-teal-600 text-white font-semibold text-[15px] shadow-[0_4px_12px_rgba(13,148,136,0.2)] active:scale-[0.98] transition-transform"
          whileTap={{ scale: 0.98 }}
        ...
        {ratings.length === 0 ? (
          ...
          <div className="space-y-2.5">
            {ratings.map((rating, i) => (
              <motion.div
                key={rating.id}
                className="bg-white rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.04)] p-4"
